...
watch(() => app.activeJobs.length, (n) => { document.title = (n ? `(${n}) ` : '') + 'Audiobook Studio · prototype' }, { immediate: true })

// a book's run finished (it had active jobs, now none) → toast, and a browser notification if enabled
const activeByBook = computed(() => { const m = {}; for (const j of app.activeJobs) m[j.bookId] = (m[j.bookId] ?? 0) + 1; return m })
watch(activeByBook, (now, before) => {
  for (const id of Object.keys(before ?? {})) {
    if (now[id]) continue
    const b = app.bookById(id); if (!b) continue
...
function onKey(e) {
  ...
  if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'z' && !e.shiftKey) { if (app.undoLast()) e.preventDefault() }
  else if (e.key === '?' || (e.shiftKey && e.key === '/')) { e.preventDefault(); shortcuts.value = true }
}
const openShortcuts = () => { shortcuts.value = true }
onMounted(() => { window.addEventListener('keydown', onKey); window.addEventListener('open-shortcuts', openShortcuts) })
onUnmounted(() => { window.removeEventListener('keydown', onKey); window.removeEventListener('open-shortcuts', openShortcuts) })
...
<template>
  ...
  <div class="flex h-screen">
    ...
    <div v-if="drawer" class="fixed inset-0 z-40 bg-black/40 lg:hidden" @click="drawer = false"></div>
    <aside class="fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col border-r border-zinc-200 bg-white transition-transform lg:static lg:w-56 lg:translate-x-0 dark:border-zinc-800 dark:bg-zinc-900" :class="drawer ? 'translate-x-0' : '-translate-x-full'">
      ...
      <nav class="mt-2 flex flex-col gap-0.5 px-2">
        <template v-for="(s, i) in stages" :key="s.key">
          <RouterLink
            ...
            <span v-if="p && s.key === 'narration'" class="text-[11px] text-zinc-400"><span v-if="p.stale" class="mr-1 text-amber-600">{{ p.stale }} stale</span>{{ p.narrated }}/{{ p.total }}</span>
      ...
      <div class="mx-2 mt-3 border-t border-zinc-200 pt-3 dark:border-zinc-800">
        <RouterLink to="/queue" class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800" :class="activeKey === 'queue' && 'bg-violet-50 font-semibold text-violet-700 dark:bg-violet-500/10 dark:text-violet-300'">
          ...
          <span v-if="app.activeJobs.length" class="flex items-center gap-1 text-[11px] text-emerald-500"><span class="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500"></span>{{ app.activeJobs.length }}<span v-if="app.eta" class="text-zinc-400"> · ~{{ Math.max(1, Math.round(app.eta.seconds / 60)) }}m</span></span>
        </RouterLink>
      </div>
      ...
      <div v-if="app.book" class="mx-3 mt-5 rounded-lg border border-zinc-200 p-3 text-xs dark:border-zinc-800">
        ...
        <div v-if="app.book.budget?.paused" class="mt-1 rounded bg-amber-400/15 px-1.5 py-0.5 text-[11px] text-amber-700 dark:text-amber-300">❚❚ paused · <button class="underline" @click="app.resumeBook(app.book.id)">resume</button></div>
        <div class="mt-2 flex flex-wrap gap-1.5">
          ...
          <RouterLink :to="`/book/${app.book.id}/search`" class="rounded border border-zinc-200 px-2 py-0.5 hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800" :class="activeKey === 'search' && 'border-violet-400'">⌕ Search</RouterLink>
        </div>
      </div>

      <div class="mt-auto flex gap-2 p-3">
        <button class="btn-ghost flex-1 justify-center" @click="app.dark = !app.dark">{{ app.dark ? '☀ Light' : '☾ Dark' }}</button>
        <button class="btn-ghost" title="keyboard shortcuts (?)" @click="shortcuts = true">?</button>
      </div>
    </aside>

    <div class="flex min-w-0 flex-1 flex-col">
      <header class="flex h-12 shrink-0 items-center justify-between gap-2 border-b border-zinc-200 bg-white px-3 sm:px-5 dark:border-zinc-800 dark:bg-zinc-900">
        <div class="flex min-w-0 items-center gap-2 text-sm text-zinc-500">
          <span class="lg:hidden"><button class="btn-ghost btn-xs" aria-label="menu" @click="drawer = true">☰</button></span>
        ...
        <div class="flex items-center gap-2">
          <button class="flex items-center gap-2 rounded-md border border-zinc-200 px-2.5 py-1 text-xs text-zinc-500 hover:border-violet-400 hover:text-violet-500 dark:border-zinc-700" @click="palette.open = true">⌕<span class="hidden sm:inline"> Jump or run…</span> <kbd class="hidden rounded border border-zinc-200 px-1 font-mono text-[10px] sm:inline dark:border-zinc-700">{{ modKey }} K</kbd></button>
          <JobIndicator />
        </div>
      </header>
