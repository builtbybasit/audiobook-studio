...
function toggle(id, e) {
  ...
  if (e?.shiftKey && lastClicked.value != null) {
    const ids = chapters.value.filter(props.selectable).map(c => c.id)
...
function pending() { emit('update:modelValue', chapters.value.filter(c => props.selectable(c) && !isDone(c)).map(c => c.id)) }
function volState(v) {
...
const counts = computed(() => {
  const out = { done: 0, failed: 0, running: 0, stale: 0, fallback: 0 }
  ...
  <div class="card flex h-full flex-col">
    <div class="flex items-center justify-between border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">
      <div>
        ...
        <div class="text-xs text-zinc-500">{{ counts.done }} done · {{ counts.failed }} failed<span v-if="counts.stale"> · <span class="text-amber-600">{{ counts.stale }} stale</span></span> · {{ modelValue.length }} selected</div>
      </div>
    ...
    <div class="border-t border-zinc-200 p-2 dark:border-zinc-800">
      <div class="mb-1 text-center text-[10px] text-zinc-400">shift-click selects a range</div>
