...
<template>
  <PopoverRoot>
    <PopoverTrigger
      class="btn-ghost btn-xs font-serif data-[state=open]:bg-zinc-200 dark:data-[state=open]:bg-zinc-800"
    ...
    <PopoverPortal>
      <PopoverContent align="end" :side-offset="6" class="ui-popup w-64 p-3 text-xs">
        ...
        <div class="mb-1 flex justify-between">
          <span>Size</span><span class="font-mono text-zinc-500">{{ reader.size }}px</span>
        </div>
        <UiSlider
          v-model="reader.size"
          :min="13"
        ...
        <div class="mb-1 flex justify-between">
          ...
          ><span class="font-mono text-zinc-500">{{ reader.lineHeight.toFixed(2) }}</span>
        </div>
        <UiSlider
          v-model="reader.lineHeight"
          :min="1.3"
          :max="2.2"
          :step="0.05"
          label="Line height"
        ...
        <div
          class="rounded-md bg-zinc-50 p-2 dark:bg-zinc-800/60"
