...
export function usePlayer() {
  ...
  function play(id, len) {
    ...
    if (p.id !== id) { p.pos = 0 }
    p.id = id; p.len = len; p.playing = true
    clearInterval(t)
    t = setInterval(() => { p.pos += 0.1; if (p.pos >= p.len) { p.pos = p.len; pause() } }, 100)
  }
  function pause() { p.playing = false; clearInterval(t) }
  function seek(frac) { p.pos = frac * p.len }
