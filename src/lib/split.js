...
function lastBoundary(text, max, mode) {
  ...
  const re = mode === 'sentence' ? /[.!?…]+["'’”)\]]*\s+/g
    : mode === 'clause' ? /(?:[.!?…]+["'’”)\]]*|[,;:—–])\s+/g
    : mode === 'word' ? /\s+/g : null
  if (!re) return max
  let best = -1, m
  while ((m = re.exec(win))) { const end = m.index + m[0].length; if (end <= max && end > 0) best = end }
  return best
}
...
export function splitText(text, maxChars, mode = 'sentence') {
  if (!maxChars || text.length <= maxChars) return [{ text, from: 0, to: text.length, at: null, fallback: false }]
  const parts = []
  let pos = 0
  ...
  while (text.length - pos > maxChars) {
    ...
    for (let i = start; i < CHAIN.length; i++) {
      ...
      if (cut > 0) { used = CHAIN[i]; break }
    }
    const piece = rest.slice(0, cut)
    parts.push({ text: piece.trimEnd(), from: pos, to: pos + piece.trimEnd().length, at: used, fallback: used !== mode })
    pos += cut
    while (text[pos] === ' ') pos++
  }
...
export const partsFor = (text, ep) => ep?.maxChars ? splitText(text, ep.maxChars, ep.splitAt ?? 'sentence').length : 1
