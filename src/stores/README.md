...
- **A settled request is a fact about the past, and it is kept as one.** `usage.ts` holds one
  ...
  derived from it (`jobs.spent`), and so is the Endpoints page's account of this session's own work. With a server answering, the ledger is the server's `requests` table and `jobs.spent` is its total.
...
`library.ts` is the first store to do this. It asks [src/services/library.ts](../services/library.ts) for a service and takes one of two halves: with a service answering, every change is a request and what comes back is what the store holds; without one, the seeded world in the store is the library. The halves are in the store rather than in the views, and the demo scenarios keep a synchronous path of their own (`_importedLocally`) because a scenario builds its situation in one pass. What the backend half cannot do — undo a removal, which therefore asks first there — is listed in [backend](../../docs/backend.md#what-is-not-done-yet) rather than papered over. A book's settings are the server's too, and each is written the way its control is used. The budget cap, the pause, the scripting budget, the pacing and a volume's name are inputs, so they change here at once and the write follows (`_writeSettings`, last one wins, so a late answer never puts back a value already typed past); what the server answers is what the store holds, and a refused write reads the book back. Changing the pacing re-times here only the chapters whose clips have been read, and takes every narrated chapter's length from the server's answer, which re-timed them from their clips. Moving a volume is the other shape, the same as removing one: it asks the server first, since it renumbers everything filed under a chapter number and the server refuses it while a build or a volume's review is open, and only then runs `_renumber` here by the same rule and installs the book and chapters the server answered with — so a job moving, which reads the book again, finds the order already there. An undo of a skip or a keep is exact on both sides: `_decisionsOf` records what the chapters were and `_restoreDecisions` puts that back, in memory or through the server's decisions route, rather than running the inverse rule.

`jobs.ts`, `scripting.ts` and `scripts.ts` follow it for the queue, through [src/services/jobs.ts](../services/jobs.ts). With a service answering: the jobs store starts empty and the queue is read into it by `useBookJobs`, which polls while anything is live; `runScripting` queues the chapters on the server, marks nothing itself and invalidates the queue; and a job that moved is noticed by that query, which reads the chapter's book again and invalidates what a finished run wrote. Cancel, remove and clear are requests, never local marks — a cancel that did not reach the server must not look like one that worked. The simulators, `demoKick`, `abandonRuns` and the scenario appliers are demo-only and never run with a service answering; the estimate and budget gates in `runScripting` are the demo's and are skipped in backend mode, where the server holds the budget — a 409 before anything is queued, a stop before a request that no longer fits — and the jobs store's `spent`, `reserved`, `scriptSpent` and `scriptReserved` answer with the server's `BookSpend` (see [backend](../../docs/backend.md#what-a-request-costs-and-what-a-book-may-spend)).

`endpoints.ts` follows it for the configuration, through [src/services/endpointSettings.ts](../services/endpointSettings.ts): the speech endpoints, the scripting profiles and the credential registry in [lib/credentials.ts](../lib/credentials.ts), which a narration job on the server reads to know which endpoint a line goes to and at what sample rate. `load` reads it when the app starts, next to the library, and installs it — an endpoint that survives keeps its object and its request telemetry, and the registry is refilled in place; a server that has never been given one (`saved: false`) is given the seeded configuration the store started with, so a first run adopts the demo's endpoints. After that the store **writes behind**: the page binds its fields straight onto the endpoint objects, so rather than a request per action the store watches one serialized projection of the configuration — the endpoints without `history`, `failures`, `rateLimits`, `backoffUntil`, `lastError` and `fetching`, the profiles and the registry — and sends the whole of it a short while (`WRITE_DELAY_MS`) after it last changed. A change that is the server's answer being installed is not sent back: the projection of what the server last described is held, and the watch sends nothing for it. Only the answer to the latest change is installed, and a refused write is said and the configuration read back. Demo mode never reads, watches or sends; the seeded configuration is simply what the page edits. A key is the exception to writing behind: the server keeps it and never sends it back, so `saveKey` writes at once with `apiKey` on that one endpoint, and no store object ever holds a key — every other write leaves `apiKey` out, which the server reads as "keep", and `hasKey` from the answer is what the page shows. `testSaved` sends any write still waiting and then asks the server to test the saved endpoint; `keyInPlace` is `hasKey` with a server and the keyring without.

## Reads are queries

**A store owns state and every change to it; a query is how a page asks for what a store holds, and — with a server answering — how that state is read from the server and kept fresh.** [src/queries/](../queries/) holds one composable per resource, on [Pinia Colada](https://pinia-colada.esm.dev): `useChapterText`, `useChapterScript`, `useChapterHistory`, `useCast`, `useBookExports`, `useBookJobs`, `useBookSpend`, `useLibrarySpend` and `useEndpointHistory`. Each takes the mode choice once — the service when there is one, the seeded store when there is not — so no view grows a demo-versus-real branch, and each installs what it reads into the store that owns it (`_install`), so the store stays the working copy every page reads and every edit acts on. A view reads the composable, or the store; it never reads the service. Keys are filed under the book ([keys.ts](../queries/keys.ts)), and a store that changes something a query reads invalidates it by key through `invalidate` — a renumbering or a removal invalidates everything under the book (`library._forgetBook`), a queued or cancelled job invalidates the queue (`jobs._changed`), and a finished scripting job invalidates the chapter's script and history and the book's cast; any job that moved invalidates its book's spending and the endpoints' requests, and a budget written invalidates the spending. There is no hand-rolled cache beside the query cache: the old `texts`/`loadText`, `_loaded`/`loadScript` and the jobs store's own polling are gone, and a new read belongs in `src/queries/`, not in a store.

**Writes are the store's, and in backend mode they are write-behind.** An edit in `scripts.ts` acts on the store exactly as it does in the demo and ends with `_commit`, which writes the chapter's script as it now stands with the revision it was read at (`_revision`); writes for one chapter are serialised and coalesced, a batch driving the per-line actions inside `history.silence()` commits once under its own name (a flag batch, which changes nothing a version keeps, commits as a plain write), and a refused write — the server's script moved — reads the server's script and history back over the local ones, directly through the service rather than by invalidating a query entry that may no longer exist, and says so. A flag, an expression and a clip's status live on the script's lines, so `narration.ts` commits them the same way; the revision is never written backwards, whichever answer lands last; and an undo of a restore waits for its script write to land before it takes the speakers the restore added off the cast. History is the server's there: `noteEdit`, `noteBulk` and `_capture` do nothing with a service answering, an edit's answer carries the history it added to, and `_chapterSnapshot` has nothing to put back. The rule behind every entry is one pure function, `planCapture` in [lib/scriptHistory.ts](../lib/scriptHistory.ts), which the store and the server both apply. The cast store's changes are requests too — a speaker written as stated (`_push`), the dictionary written whole (`_pushLexicon`), a rename, a merge or a removal that moves lines on the server and applies what moved here (`_moved`) — and their undo is exact on both sides: a rename renames back, and a merge or a removal puts back exactly the lines that moved through `attribute`, the same shape as the review's `_restoreDecisions`. The dictionary is exact the same way: the server stales every clip whose recorded pronunciation the new list no longer sends, by the rule `_lexRestale` applies here, and answers with those lines and each chapter's new revision, which `_pushLexicon` adopts (never backwards); the Undo waits for that answer, reverts, and sends the old list naming exactly those lines, and the server puts back to done the ones that read it again. Narration is the server's too: `runNarration` queues the chapters at a scope and marks nothing, the queue's poll reads a chapter's script again as each clip lands (a read that moves the revision without a script change is not a re-script, so `_install` keeps no diff for it), a retake queues the lines named on the server, and its verdict replaces the line and the chapter with the server's answer, without an Undo, since the displaced clip is a take. Building an audiobook is the server's as well: `buildExport` sends the selection and the settings, marks nothing of its own, and adds the entry the server answers with to the book's list — the refusals are the server's too, in the same words the page's blocker panel uses, so the store does not ask the same questions a second time. A build names its cover by a url the server gave for the book, never by the image itself, so `chooseCover` uploads a picked JPEG or PNG through `uploadCover` and puts the url it answers with in the settings — a refusal, the server's or the up-front check on the file's type, leaves the cover as it was; the demo holds the image as a `data:` URL instead. A build's job moving is what keeps the Audiobooks tab current: the queue's poll invalidates the book's exports on every move, which is also how a cancel — the server deletes the row it was writing — reaches the page. What the backend half cannot do exactly — an undo of an edit, a bulk correction or a restore is an edit that writes the previous script back — is listed in [backend](../../docs/backend.md#what-is-not-done-yet).
