...
`jobs.ts`, `scripting.ts` and `scripts.ts` follow it for the queue, through [src/services/jobs.ts](../services/jobs.ts). With a service answering: the jobs store starts empty, reads the queue from the server and polls it while anything is live (`startPolling`, started once at startup and again by each enqueue, stopping itself when the queue goes quiet); `runScripting` queues the chapters on the server and marks nothing itself; and `refresh` is where a job that moved becomes a chapter read again and a finished script read into `scripts.segments` (`loadScript`). Cancel, remove and clear are requests, never local marks — a cancel that did not reach the server must not look like one that worked. The simulators, `demoKick`, `abandonRuns` and the scenario appliers are demo-only and never run with a service answering; the estimate and budget gates in `runScripting` are the seeded endpoints' and are bypassed in backend mode, which [backend](../../docs/backend.md#the-queue) records as a limitation rather than a rule.
