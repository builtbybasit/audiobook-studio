...
const inner = computed({
  get: () => props.action ? null : props.options.find(o => o.value === props.modelValue) ?? null,
...
function onKey(e) {
  ...
  if (e.key === 'Enter' && open.value) {
    // reka commits the highlighted item; if nothing is highlighted, take the first match
    const highlighted = document.querySelector('[role=listbox] [data-highlighted]')
    if (highlighted) return
    const first = groups.value[0]?.[1]?.[0]
    if (first) { e.preventDefault(); inner.value = first; open.value = false; e.target.blur() }
  }
}
...
<template>
  <ComboboxRoot v-model="inner" v-model:open="open" :ignore-filter="true" :reset-search-term-on-blur="true" :class="block && 'w-full'">
    <ComboboxAnchor class="ui-select-trigger" :class="[size === 'xs' ? 'py-0.5 text-xs' : 'py-1 text-sm', block && 'w-full']">
      ...
      <ComboboxInput v-model="term" :display-value="o => o?.label ?? ''" :placeholder="action ? placeholder : (inner?.label ?? placeholder)" class="min-w-0 flex-1 bg-transparent placeholder-zinc-400 focus:outline-none" :class="action && 'italic'" @focus="e => { e.target.select(); open = true }" @keydown="onKey" />
    ...
    <ComboboxPortal>
      <ComboboxContent position="popper" :side-offset="4" class="ui-popup" :style="{ minWidth: 'var(--reka-combobox-trigger-width)', maxHeight: '320px' }">
        <ComboboxViewport class="p-1">
          <ComboboxEmpty class="px-2 py-2 text-xs text-zinc-500">No match.</ComboboxEmpty>
