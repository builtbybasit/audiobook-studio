...
const meta = reactive({
  ...
  cover: null, markers: true, markerPattern: '{n}. {title}',
})
const { p: player, play } = usePlayer()
const PATTERNS = [{ value: '{n}. {title}', label: '1. The Silent Peak' }, { value: 'Chapter {n} — {title}', label: 'Chapter 1 — The Silent Peak' }, { value: '{title}', label: 'The Silent Peak' }, { value: 'Chapter {n}', label: 'Chapter 1' }]
...
<template>
  <div class="grid gap-4 p-4 lg:h-full lg:grid-cols-[300px_1fr] lg:grid-rows-[minmax(0,1fr)]">
    <div class="h-[50vh] min-h-0 lg:h-auto"><ChapterPicker :book-id="bookId" stage="export" v-model="selected" run-label="Build audiobook" :selectable="c => isNarrated(c) && !c.excluded" @run="ids => app.buildExport(bookId, ids, meta)" /></div>
    ...
    <div v-else class="grid min-h-0 gap-4 lg:grid-cols-[1fr_340px] lg:grid-rows-[minmax(0,1fr)]">
      ...
      <TabsRoot v-model="tab" class="card flex min-h-0 flex-col">
        ...
        <div class="min-h-0 flex-1 overflow-auto p-5">
          ...
          <TabsContent value="metadata" class="space-y-4">
            <div class="grid gap-3 text-sm sm:grid-cols-2">
              ...
              <label class="sm:col-span-2">Description<textarea v-model="meta.description" rows="2" class="input mt-1 w-full" placeholder="Shown in audiobook players. Leave blank to use the EPUB blurb."></textarea></label>
            </div>
            ...
            <div class="flex items-center gap-4">
              <img v-if="meta.cover" :src="meta.cover" class="h-24 w-16 shrink-0 rounded-md object-cover" alt="cover" />
              <div v-else class="h-24 w-16 shrink-0 rounded-md" :style="{ background: `linear-gradient(160deg, ${book.cover[0]}, ${book.cover[1]})` }"></div>
              <div class="text-sm"><div class="font-medium">Cover</div><div class="text-xs text-zinc-500">{{ meta.cover ? 'Your image, embedded in every file.' : 'From the EPUB, embedded in every file.' }}</div>
                <div class="mt-2 flex gap-1"><label class="btn-ghost btn-xs cursor-pointer">{{ meta.cover ? 'Replace…' : 'Use my own…' }}<input type="file" accept="image/*" class="hidden" @change="pickCover" /></label><button v-if="meta.cover" class="btn-ghost btn-xs" @click="meta.cover = null">Back to EPUB cover</button></div></div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="chapters">
            <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
              ...
              <div class="flex flex-wrap items-center gap-3 text-xs">
                ...
                <UiSelect v-model="meta.markerPattern" :options="PATTERNS" size="xs" class="w-52" :disabled="!meta.markers" />
                <UiSwitch v-if="multi && !meta.splitPerVolume" v-model="meta.volPrefix" label="prefix with volume" class="text-zinc-500" />
              </div>
            </div>
            <div v-if="!meta.markers" class="mb-2 rounded-md bg-amber-400/10 px-2 py-1 text-xs text-amber-700 dark:text-amber-300">Without markers the player shows one long track — listeners can’t skip by chapter.</div>
            <div v-else class="mb-2 rounded-md bg-zinc-50 px-2 py-1.5 font-mono text-[11px] text-zinc-500 dark:bg-zinc-800/60">as the player will list them: <span v-for="(c, i) in selChapters.slice(0, 3)" :key="c.id" class="mr-3 text-zinc-800 dark:text-zinc-200">{{ marker(c, i) }}</span><span v-if="selChapters.length > 3">…</span></div>
        ...
        <div class="border-t border-zinc-200 px-5 py-3 dark:border-zinc-800">
          ...
          <div v-else class="mt-1 text-[11px] text-zinc-400">{{ meta.markers ? `${selChapters.length} chapter markers` : 'no chapter markers' }} · {{ meta.cover ? 'custom cover' : 'EPUB cover' }} · {{ meta.bitrate }} kbps AAC</div>
        </div>
      </TabsRoot>
      ...
      <div class="card min-h-0 overflow-auto p-4">
        ...
        <div v-for="e in exportsHere" :key="e.id" class="mb-3 rounded-lg border border-zinc-200 p-3 text-sm dark:border-zinc-800">
          ...
          <template v-else>
            ...
            <div class="mt-2 flex items-center gap-1 rounded bg-zinc-50 px-2 py-1 font-mono text-[10px] text-zinc-500 dark:bg-zinc-800/60"><span class="min-w-0 flex-1 truncate" :title="pathOf(e)">{{ pathOf(e) }}</span><button class="shrink-0 text-violet-500 hover:underline" @click="copyPath(e)">copy</button></div>
            <div class="mt-2 flex items-center gap-2 text-xs text-zinc-500">
              <span>{{ e.size }} MB · {{ e.createdAt }}</span>
              <span class="ml-auto flex gap-1">
                <button class="btn-ghost btn-xs" title="listen (prototype player)" @click="play('exp' + e.id, e.duration)">{{ player.id === 'exp' + e.id && player.playing ? '❚❚' : '▶' }}</button>
            ...
            <div v-if="player.id === 'exp' + e.id" class="mt-2 h-1 rounded bg-zinc-200 dark:bg-zinc-800"><div class="h-1 rounded bg-violet-500" :style="{ width: (player.len ? player.pos / player.len * 100 : 0) + '%' }"></div></div>
