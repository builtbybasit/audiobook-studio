  ...
  <div class="grid gap-4 p-4 lg:h-full lg:grid-cols-[300px_1fr] lg:grid-rows-[minmax(0,1fr)]">
    ...
    <div class="min-h-0 min-w-0 lg:h-full">
      ...
      <EmptyState v-else-if="!anyScripted && chapter?.scripting === 'none'" icon="✎" title="Nothing scripted yet"
        ...
        :steps="['Pick a profile and chunk size below the chapter list', 'Tick chapters (or use Pending) and run', 'Review the cast and merge aliases before narrating']">
        <button class="btn-primary" @click="scriptFirst">Script the first 3 chapters</button>
      </EmptyState>
      <EmptyState v-else-if="chapter?.scripting === 'running' || chapter?.scripting === 'queued'" icon="✎" :title="chapter.title" :body="chapter.scripting === 'queued' ? 'Queued — waiting for the previous chapter so context carries forward.' : `Extracting segments… ${Math.round(chapter.scriptingProgress)}%`" />
      <EmptyState v-else-if="chapter?.scripting === 'failed'" icon="⚠" :title="chapter.title"
        ...
        <button class="btn-primary" @click="app.runScripting(bookId, [opened])">Re-run this chapter</button>
        <button class="btn-ghost" @click="app.scriptSettings.chunkChars = Math.max(2000, app.scriptSettings.chunkChars - 2000); app.runScripting(bookId, [opened])">Smaller chunks + re-run</button>
      </EmptyState>
      <EmptyState v-else icon="✎" :title="chapter?.title" body="Not scripted yet. Tick it on the left and run scripting, or script just this one.">
        <button class="btn-primary" @click="app.runScripting(bookId, [opened])">Script this chapter</button>
      </EmptyState>
    </div>
  </div>
</template>
