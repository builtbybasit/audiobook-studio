...
function drift(s: Segment) {
  ...
  const now = app.effectiveVoice(props.bookId, s.speaker);
  if (a.voiceRef && now.ref !== a.voiceRef)
    out.push(`voice: ${app.voiceLabel(a.voiceRef)} → ${app.voiceLabel(now.ref) || "unset"}`);
  const who = app.charactersOf(props.bookId).find((c) => c.name === s.speaker);
...
<template>
  <div class="card flex h-full min-h-0 flex-col">
    <div
      class="grid grid-cols-3 divide-x divide-zinc-200 border-b border-zinc-200 sm:grid-cols-6 dark:divide-zinc-800 dark:border-zinc-800"
    >
      <button
        v-for="f in FILTERS"
        :key="f"
      ...
      >
        ...
        <div
          ...
          :class="{
            ...
            'text-amber-500': f === 'stale' && count(f),
          }"
        >
          {{ count(f) }}
        </div>
    ...
    >
      <span class="text-zinc-500"
        >{{ chapter.title }} · {{ fmt(total) }} · {{ chapter.narration }}</span
      >
      <span class="ml-auto flex gap-2">
        <button
          v-if="count('stale') && chapter.narration !== 'running'"
          class="btn-primary btn-xs"
          @click="app.renarrateStale(bookId, chapterId)"
        >
          ↻ Re-narrate changed ({{ count("stale") }})
        </button>
        <button
      ...
      <table class="w-full min-w-[640px] table-fixed text-sm">
        ...
        >
          <tr>
            ...
            <th class="w-6"></th>
            <th class="w-32">Speaker</th>
            <th>Text</th>
            <th class="w-28">Endpoint</th>
            <th class="w-14 text-right">Took</th>
            <th class="w-14 text-right">Audio</th>
            <th class="w-28"></th>
          </tr>
        </thead>
        <tbody>
          <template v-for="s in rows" :key="s.id">
            <tr
              ...
              :class="currentId === s.id && 'bg-violet-50 dark:bg-violet-500/10'"
            ...
            >
              ...
              <td class="py-1.5">
                ...
                <div class="truncate pl-3.5 text-[10px] text-zinc-400">
                  {{ voiceOf(s.speaker) }}
                </div>
              </td>
              <td
                class="truncate py-1.5 pr-3 text-zinc-600 dark:text-zinc-300"
              ...
              >
                ...
                <div class="flex flex-wrap gap-2 text-[10px]">
                  ...
                  ><span v-if="s.audio.error" class="text-red-500"
              ...
              <td class="text-right font-mono text-xs text-zinc-500">
                {{ s.audio.ms ? (s.audio.ms / 1000).toFixed(1) + "s" : "" }}
              </td>
              <td class="text-right font-mono text-xs text-zinc-500">
              ...
              <td class="pr-3 text-right whitespace-nowrap">
                <template v-if="s.audio.status === 'done' || s.audio.status === 'stale'">
                  ...
                  >
                    {{ p.id === "seg" + s.id && p.playing ? "❚❚" : "▶" }}
                  </button>
                  <button
                    class="ml-1 text-[11px] text-zinc-400 hover:text-violet-500"
                    title="play chapter from here"
                    @click="playFrom(s)"
                  >
                    ⇥
                  </button>
                ...
                >
                <button
                  v-if="s.audio.at || s.audio.error || s.audio.cuts"
                  class="ml-1 rounded border px-1 text-[10px]"
                  :class="
                    expanded.has(s.id)
                      ? 'border-violet-400 text-violet-500'
                      : 'border-zinc-300 text-zinc-400 hover:text-violet-500 dark:border-zinc-700'
                  "
                  title="render details (i)"
            ...
            <tr v-if="expanded.has(s.id)" class="bg-zinc-50 dark:bg-zinc-900/60">
              ...
              <td colspan="6" class="px-2 py-2 pr-4 text-xs">
                <div v-if="s.audio.at" class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5">
                  <span class="text-zinc-400">rendered</span
                  ><span
                    >{{ clock(s.audio.at) }} · {{ epName(s.audio.endpoint) }} ·
                    <span class="font-mono">{{ s.audio.model }}</span> ·
                    {{ s.audio.ms ? (s.audio.ms / 1000).toFixed(1) + "s" : ""
                    }}<span v-if="s.audio.cost"> · ${{ s.audio.cost.toFixed(4) }}</span></span
                  >
                  <span class="text-zinc-400">voice</span
                  ><span>{{ app.voiceLabel(s.audio.voiceRef) || s.audio.voice }}</span>
                  <span class="text-zinc-400">direction</span
                  ><span
                    >{{ s.audio.direction || "—"
                    }}<span v-if="s.audio.style" class="text-zinc-500">
                      · style: {{ s.audio.style }}</span
                    >
                    · {{ s.audio.type }}</span
                  >
                  <template v-if="drift(s).length"
                    ><span class="text-amber-600">differs now</span
                    ><span class="text-amber-600">{{ drift(s).join(" · ") }}</span></template
                  >
                </div>
                <div
                  v-if="s.audio.error"
                  class="mt-1 rounded border border-red-300 bg-red-500/5 p-2 dark:border-red-500/40"
                >
                  <div class="flex items-center gap-2">
                    <b class="text-red-600">{{
                      s.audio.error.code ? "HTTP " + s.audio.error.code : "not sent"
                    }}</b
                    ><span>{{ s.audio.error.message }}</span
                    ><span v-if="s.audio.error.at" class="text-zinc-400"
                      >· {{ clock(s.audio.error.at) }}</span
                    ><button class="ml-auto text-violet-500 hover:underline" @click="copyReq(s)">
                      copy request
                    </button>
                  </div>
                  <pre
                    v-if="s.audio.error.body"
                    class="mt-1 max-h-20 overflow-auto whitespace-pre-wrap break-all rounded bg-white p-1.5 font-mono text-[10px] text-zinc-600 dark:bg-zinc-900 dark:text-zinc-400"
