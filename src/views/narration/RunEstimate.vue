...
<template>
  <div class="text-xs">
    ...
    <div class="grid grid-cols-2 gap-x-4 gap-y-1">
      ...
      <span class="text-zinc-500">Characters</span><span class="text-right font-mono">{{ k(est.chars) }}</span>
      <span class="text-zinc-500">Requests</span><span class="text-right font-mono">{{ est.requests }}<span v-if="est.split" class="text-amber-600" :title="`${est.split} segments exceed their endpoint’s per-request limit and are sent in parts`"> · {{ est.split }} split</span></span>
    ...
    <div v-if="est.per.length" class="mt-2 border-t border-zinc-100 pt-1.5 dark:border-zinc-800">
      ...
      <div v-for="e in est.per" :key="e.endpoint.id" class="flex items-center gap-2 py-0.5">
        <span class="h-1.5 w-1.5 shrink-0 rounded-full" :class="e.endpoint.enabled ? 'bg-emerald-500' : 'bg-zinc-400'"></span>
        <span class="min-w-0 flex-1 truncate" :class="!e.endpoint.enabled && 'text-zinc-400 line-through'">{{ e.endpoint.name }}</span>
        <span class="font-mono text-zinc-500" :title="`${e.segments} segments · ${e.requests} requests${e.endpoint.maxChars ? ' · limit ' + e.endpoint.maxChars : ''}`">{{ e.requests }} req</span>
        <span class="w-12 text-right font-mono text-zinc-500">{{ e.endpoint.price ? '$' + (e.chars / 1e6 * e.endpoint.price).toFixed(2) : 'free' }}</span>
      </div>
      <div v-if="est.unrouted" class="mt-0.5 text-amber-600">{{ est.unrouted }} segments have no endpoint (voice missing).</div>
    </div>
