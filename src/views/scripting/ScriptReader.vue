...
const dirOpts = computed(() => {
  const used = new Map()
  for (const k of Object.keys(app.segments)) if (k.startsWith(props.bookId + ':')) for (const x of app.segments[k]) if (x.direction) used.set(x.direction, (used.get(x.direction) ?? 0) + 1)
  const fromBook = [...used.entries()].sort((a, b) => b[1] - a[1]).map(([d, n]) => ({ value: d, label: d, group: 'Used in this book', hint: n + '×' }))
  return [...fromBook, ...DIRECTIONS.filter(d => !used.has(d)).map(d => ({ value: d, label: d, group: 'Presets' }))]
})
const sameSpeakerCount = (s) => segments.value.filter(x => x.speaker === s.speaker && x.id !== s.id).length
...
function rescript() { rescriptOpen.value = false; app.runScripting(props.bookId, [props.chapterId], { keepEdits: keepEdits.value }) }
function jumpTo(id) { focus.value = id; open.value = null; nextTick(() => document.getElementById('seg-' + id)?.scrollIntoView({ block: 'center', behavior: 'smooth' })) }
...
function onKey(e) {
  ...
  else if (e.key === '/' && !e.shiftKey) { e.preventDefault(); document.querySelector('input[placeholder^="Find chapter"]')?.focus() }
...
onMounted(() => { window.addEventListener('keydown', onKey); if (route.query.seg) jumpTo(Number(route.query.seg)) })
onUnmounted(() => window.removeEventListener('keydown', onKey))
watch(() => route.query.seg, v => { if (v) jumpTo(Number(v)) })
watch(open, v => { if (v) focus.value = v })
</script>

<template>
  <div class="grid h-full gap-4" :class="reader.showCast ? 'lg:grid-cols-[1fr_300px]' : 'grid-cols-1'">
    <!-- reader -->
    <div class="card flex min-h-0 min-w-0 flex-col">
      <div class="border-b border-zinc-200 px-4 pb-3 pt-4 sm:px-6 dark:border-zinc-800">
        <div class="flex flex-wrap items-start gap-2">
          ...
          <PopoverRoot v-model:open="rescriptOpen">
            <PopoverTrigger class="btn-ghost btn-xs" title="run the LLM again on this chapter">↻ Re-script</PopoverTrigger>
            <PopoverPortal>
              <PopoverContent :side-offset="6" align="end" class="ui-popup w-80 p-3 text-xs">
                ...
                <div class="grid grid-cols-[auto_1fr] items-center gap-x-2 gap-y-1.5">
                  <span class="text-zinc-500">Profile</span><UiSelect v-model="app.scriptSettings.profile" :options="app.profiles.map(p => ({ value: p.id, label: p.name, hint: p.model }))" size="xs" block />
                  <span class="text-zinc-500">Chunk</span><div class="flex items-center gap-2"><input v-model.number="app.scriptSettings.chunkChars" type="number" step="500" min="2000" class="input w-24 py-0.5 font-mono" /><span class="text-zinc-400">chars</span></div>
                </div>
                <div class="mt-2 rounded-md bg-zinc-50 p-2 dark:bg-zinc-800/60">
                  <template v-if="edits"><UiSwitch v-model="keepEdits" :label="`keep my ${edits} manual edit${edits === 1 ? '' : 's'}`" /><div class="mt-1 text-[11px] text-zinc-500">{{ keepEdits ? 'Speaker / type / direction you changed are re-applied where the text still matches.' : 'Your edits are discarded — the new run wins.' }}</div></template>
                  <div v-else class="text-[11px] text-zinc-500">No manual edits in this chapter.</div>
                </div>
                <div class="mt-1 text-[11px] text-zinc-500"><template v-if="segments.some(s => s.audio.duration)">Narrated audio is kept; lines whose speaker or direction change become stale.</template> A “what changed” panel appears when it finishes.</div>
                <div class="mt-3 flex justify-end gap-2"><button class="btn-ghost btn-xs" @click="rescriptOpen = false">Cancel</button><button class="btn-primary btn-xs" @click="rescript">Re-script now</button></div>
              </PopoverContent>
            </PopoverPortal>
          </PopoverRoot>
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2">
          ...
          <span class="ml-auto whitespace-nowrap text-[11px] text-zinc-400">{{ rows.length }} shown · click any line to edit</span>
        </div>
      </div>
      ...
      <div v-if="stale" class="flex items-center gap-3 border-b border-amber-300 bg-amber-400/10 px-6 py-2 text-xs text-amber-700 dark:border-amber-500/40 dark:text-amber-300">
        <span>♪</span>
        <span class="flex-1"><b>{{ stale }} line{{ stale > 1 ? 's' : '' }} changed since narration</b> — the audio still reads the old script.</span>
        <RouterLink :to="{ path: `/book/${bookId}/narration`, query: { ch: chapterId } }" class="btn-primary btn-xs">↻ Re-narrate changed</RouterLink>
      </div>
      <div v-if="diff && showDiff" class="border-b border-violet-300 bg-violet-50 px-6 py-2 text-xs dark:border-violet-500/40 dark:bg-violet-500/10">
        <div class="flex items-center gap-3">
          <span class="text-violet-500">↻</span>
          <span class="flex-1"><b>Re-scripted.</b> {{ diff.prevCount }} → {{ diff.curCount }} segments · <b>{{ diff.speaker.length }}</b> speaker change{{ diff.speaker.length === 1 ? '' : 's' }} · <b>{{ diff.direction.length }}</b> direction change{{ diff.direction.length === 1 ? '' : 's' }} · <b>{{ diff.added.length }}</b> new · <b>{{ diff.removed.length }}</b> gone<span v-if="!diff.total"> — identical to the previous run</span></span>
          <button class="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100" @click="app.dismissDiff(bookId, chapterId)">dismiss</button>
        </div>
        <ul v-if="diff.total" class="mt-1.5 max-h-28 space-y-0.5 overflow-auto pl-6">
          <li v-for="d in diff.speaker" :key="'s' + d.id" class="flex gap-2"><button class="font-mono text-violet-500 hover:underline" @click="jumpTo(d.id)">#{{ d.id }}</button><span class="truncate"><span class="text-zinc-500 line-through">{{ d.from }}</span> → <b>{{ d.to }}</b> · {{ d.text.slice(0, 70) }}…</span></li>
          <li v-for="d in diff.direction" :key="'d' + d.id" class="flex gap-2"><button class="font-mono text-violet-500 hover:underline" @click="jumpTo(d.id)">#{{ d.id }}</button><span class="truncate">direction <span class="text-zinc-500">{{ d.from || '—' }}</span> → <b>{{ d.to || '—' }}</b></span></li>
          <li v-for="d in diff.added" :key="'a' + d.id" class="flex gap-2"><button class="font-mono text-emerald-600 hover:underline" @click="jumpTo(d.id)">#{{ d.id }}</button><span class="truncate">new · {{ d.text.slice(0, 80) }}…</span></li>
          <li v-for="(d, i) in diff.removed" :key="'r' + i" class="flex gap-2 text-zinc-500"><span class="font-mono">—</span><span class="truncate">gone · {{ d.text.slice(0, 80) }}…</span></li>
        </ul>
      </div>
      <div class="min-h-0 flex-1 overflow-auto px-4 py-5 sm:px-6">
        <div class="mx-auto" :class="[reader.widthClass, reader.fontClass]" :style="{ fontSize: reader.size + 'px', lineHeight: reader.lineHeight }">
          <template v-for="s in rows" :key="s.id">
            ...
            <!-- inline editor -->
            <div v-if="open === s.id" class="-mt-1 mb-4 grid grid-cols-2 items-end gap-2 rounded-md border border-violet-300 bg-white p-2 font-sans text-xs leading-normal 2xl:grid-cols-[1fr_1fr_2fr_auto] dark:border-violet-500/40 dark:bg-zinc-900" @click.stop>
              ...
              <label class="col-span-2 2xl:col-span-1">Direction <span class="text-zinc-400">— pick or type your own</span>
                <div class="mt-1 flex items-center gap-1">
                  <UiCombobox :model-value="s.direction" :options="dirOpts" custom placeholder="e.g. whispered, hesitant" size="xs" class="min-w-0 flex-1" block @update:model-value="v => app.updateSegment(bookId, chapterId, s.id, { direction: v })" />
                  <UiTooltip :text="`Set “${s.direction || '—'}” on every ${s.speaker} line in this chapter (${sameSpeakerCount(s)} more)`"><button class="btn-ghost btn-xs whitespace-nowrap" :disabled="!s.direction || !sameSpeakerCount(s)" @click="app.applyDirection(bookId, chapterId, s.speaker, s.direction)">→ all {{ s.speaker.split(' ')[0] }}</button></UiTooltip>
                </div>
              </label>
              <div class="flex items-center gap-2 justify-self-end"><span v-if="s.edited" class="text-[10px] text-zinc-400">edited</span><button class="btn-ghost btn-xs" @click="open = null">Done</button></div>
            </div>
          </template>
          <div v-if="!rows.length" class="py-10 text-center text-sm text-zinc-500">Nothing matches this filter.</div>
        </div>
      </div>
      <div class="hidden border-t border-zinc-200 px-6 py-1 font-sans text-[11px] text-zinc-400 xl:block dark:border-zinc-800">
    ...
    <!-- cast rail -->
    <div v-if="reader.showCast" class="card max-h-[50vh] min-h-0 overflow-y-auto overflow-x-hidden p-3 lg:max-h-none">
      ...
      <div v-for="c in inChapter" :key="c.name" class="mb-2 rounded-lg border p-3 text-sm transition-colors"
        ...
        <div class="mt-2 flex flex-wrap items-center gap-1 pl-4 text-[11px]">
          <span class="mr-auto min-w-0 truncate text-zinc-500">voice: <b class="text-zinc-700 dark:text-zinc-300">{{ app.voiceLabel(app.effectiveVoice(bookId, c.name).ref) || 'unset' }}</b><span v-if="!app.effectiveVoice(bookId, c.name).own && app.effectiveVoice(bookId, c.name).ref"> (Narrator’s)</span></span>
          <button class="rounded px-1.5 py-0.5 hover:bg-zinc-100 dark:hover:bg-zinc-800" @click="startRename(c)">rename</button>
          <UiCombobox v-if="c.name !== 'Narrator'" action :options="cast.filter(x => x.name !== c.name).map(o => ({ value: o.name, label: o.name, color: o.color, keywords: o.aliases.join(' ') }))" placeholder="merge into…" size="xs" class="w-28" @pick="v => app.mergeCharacter(bookId, c.name, v)" />
        </div>
      </div>
