...
<template>
  <div class="text-xs">
    ...
    <div v-if="est.profile?.needsKey" class="mt-1.5 flex items-center gap-2"><span class="text-zinc-500">Key</span><input :value="keyring.get('profile:' + est.profile.id)" type="password" class="input min-w-0 flex-1 py-0.5 font-mono" :placeholder="keyring.has('profile:' + est.profile.id) ? '' : 'paste API key (kept in memory only)'" @input="keyring.set('profile:' + est.profile.id, $event.target.value)" /><span v-if="!keyring.has('profile:' + est.profile.id)" class="text-amber-600">⚠ no key</span></div>
